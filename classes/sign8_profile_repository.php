...
class sign8_profile_repository {
    public static function for_document(string $document): array {
        $issuer_id = self::issuer_id($document);
        $profiles = json_decode(get_config('local_isycredentials', 'sign8_issuer_profiles'), true);
        if (!is_array($profiles) || !isset($profiles[$issuer_id]) || !is_array($profiles[$issuer_id])) {
            throw new \moodle_exception('sign8_profile_not_found', 'local_isycredentials', '', $issuer_id);
        }
    ...
    private static function issuer_id(string $document): string {
        ...
        if (!is_array($issuer) || empty($issuer['id']) || !is_string($issuer['id'])) {
            throw new \moodle_exception('sign8_invalid_issuer', 'local_isycredentials');
        }
        return $issuer['id'];
    }
}
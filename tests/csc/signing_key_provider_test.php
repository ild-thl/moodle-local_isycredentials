...
use local_isycredentials\csc\api_exception;
use local_isycredentials\csc\http_client_interface;
use local_isycredentials\csc\signing_key_provider;

class signing_key_provider_test extends \advanced_testcase {
    public function test_signing_hash_is_authorized_signed_polled_and_revoked(): void {
        $this->resetAfterTest();
        putenv('CSC_TEST_SECRET=client-secret');
        putenv('CSC_TEST_CERT=/run/secrets/csc-client.crt');
        putenv('CSC_TEST_KEY=/run/secrets/csc-client.key');
        $client = new fake_http_client([
        ...
        $provider = new signing_key_provider($this->profile(), $client);
        ...
        $this->assertSame('/run/secrets/csc-client.crt', $client->requests[0]['tls_options']['certificate']);
    ...
    public function test_revoke_is_called_when_signing_fails(): void {
        ...
        try {
            ...
            $this->assertSame('csc_missing_request_id', $exception->errorcode);
        }
    ...
    public function test_pending_poll_is_retried_with_the_same_request_id(): void {
        ...
        $provider = new signing_key_provider($profile, $client);
    ...
    private function profile(): array {
        return [
            ...
            'redirect_uri' => 'https://moodle.example/csc',
            'sign_algo' => '1.2.840.113549.1.1.1',
            'dss_signature_algorithm' => 'RSA_SHA256',
            'client_secret_env' => 'CSC_TEST_SECRET',
            'tls_certificate_path_env' => 'CSC_TEST_CERT',
            'tls_key_path_env' => 'CSC_TEST_KEY',
        ];
    }
}
